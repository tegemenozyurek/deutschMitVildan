...
import {
  generateWFragenQuiz,
  WFRAGEN_QUIZ_COUNTS,
  type WFragenQuizCount,
  type WFragenQuizQuestion,
} from '../lib/wFragenQuiz'

type Phase = 'ready' | 'count' | 'countdown' | 'quiz' | 'done'

export function WFragenEgzersizPage() {
  const [phase, setPhase] = useState<Phase>('ready')
  const [quizCount, setQuizCount] = useState<WFragenQuizCount | null>(null)
  const [countdown, setCountdown] = useState(3)
  const [questions, setQuestions] = useState<WFragenQuizQuestion[]>([])
  ...
  useEffect(() => {
    if (phase !== 'countdown' || !quizCount) return
    setCountdown(3)
    let value = 3
    const id = window.setInterval(() => {
      ...
      if (value <= 0) {
        ...
        setQuestions(generateWFragenQuiz(quizCount))
        setIndex(0)
        setScore(0)
        setPickedId(null)
        setLocked(false)
  ...
  const goToCount = () => setPhase('count')

  const pickCount = (count: WFragenQuizCount) => {
    setQuizCount(count)
    setPhase('countdown')
  }
      ...
      <PageShell>
        ...
        <div className="relative z-10 flex w-full max-w-xl flex-1 flex-col items-center justify-center gap-8 overflow-visible py-4 animate-[rise_0.9s_0.08s_ease_both]">
          <div className="flex w-full items-center justify-center pt-28 sm:pt-32 md:pt-36">
            <LogoWithBubble message="W-Soruları egzersizine Hazırsan başlayalım!" />
          </div>
      ...
      <PageShell>
        ...
        <div className="relative z-10 flex w-full max-w-md flex-1 flex-col items-center justify-center gap-6 px-2 py-6 animate-[rise_0.9s_0.08s_ease_both]">
          <div className="text-center">
            ...
            <p className="mt-2 text-[#3a5249]">Havuzdan rastgele seçilir</p>
          </div>
          <div className="flex w-full flex-col gap-3">
            {WFRAGEN_QUIZ_COUNTS.map((count) => (
              <button
                key={count}
                type="button"
